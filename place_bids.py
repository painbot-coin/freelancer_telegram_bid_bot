...
while True:    
    ...
    if p:
        ...
        for x in p.get('projects'):
            ...
            amount = int((x.get('budget').get('maximum') - x.get('bid_stats').get('bid_avg'))/2) + x.get('bid_stats').get('bid_avg')
            
            try:
                bid = get_bids(session, **get_bids_data)
                if bid and x.get('status') == 'active' and (x.get('currency').get('code') == 'USD' or x.get('currency').get('code') == 'AUD'):
